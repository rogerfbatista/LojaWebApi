...
app.controller('produtoFornecedorEntradaController', ['$scope', '$q', '$timeout', '$sce', '$modal',
    'ProdutoFornecedorEntrada', 'Fornecedor', 'Produto', '$routeParams', '$location', 'authService',
function ($scope, $q, $timeout, $sce, $modal, ProdutoFornecedorEntrada, Fornecedor, Produto,
    ...
    $scope.adicionar = function () {
        ...
        if ($scope.userForm.$valid) {
            ...
            for (var i = 0; i < $scope.produtoFornecedorEntradas.length; i++) {
                ...
                if (produtoFornecedorEntrada.produtoId == $scope.produtoFornecedorEntradas[i].produtoId) {
    ...
    $scope.delete = function (produtoFornecedorEntrada) {
        var index = $scope.produtoFornecedorEntradas.indexOf(produtoFornecedorEntrada);
        $scope.produtoFornecedorEntradas.splice(index, 1);
    ...
    $scope.save = function () {
        ...
        if ($scope.produtoFornecedorEntradas.length > 0) {
            ...
            ProdutoFornecedorEntrada.save(JSON.stringify($scope.produtoFornecedorEntradas)).$promise.then(function () {
                $scope.produtoFornecedorEntradas = [];
                $scope.produtoFornecedorEntrada = {};
                $scope.dirty = {};
                $scope.dirty1 = {};
